class CurologyProductCarousel extends HTMLElement {
  connectedCallback() {
    this.track = this.querySelector('.curology-product-carousel__track');
    this.prevBtn = this.querySelector('[data-carousel-prev]');
    this.nextBtn = this.querySelector('[data-carousel-next]');

    if (!this.track || !this.prevBtn || !this.nextBtn) return;

    this.scrollAmount = 320; // card width (288) + gap (32)
...
customElements.define('curology-product-carousel', CurologyProductCarousel);
